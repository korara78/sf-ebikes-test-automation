...
- CI (`.github/workflows/playwright.yml`) runs on push/PR to `main`/`master`, scoped to the Guest Suite only (`--project=chromium --project=firefox --project=webkit`): the Internal Suite's `setup` project shells out to the `sf` CLI, which isn't installed/authenticated in CI. Internal Suite tests only run locally for now.
- **Auth strategy (see `guides/04-authentication-test-session-strategy.md`):** `tests/auth.setup.ts` sidesteps Salesforce login/MFA entirely by piggybacking on the `sf` CLI's existing authenticated session — `sf org open --url-only --json -p lightning` exchanges the CLI's OAuth token for a single-use frontdoor bridge URL, which Playwright visits once to capture `storageState`. It also persists the internal Lightning app's origin (a different domain than `E_BIKES_BASE_URL`) to `playwright/.auth/lightning-origin.json` via `pages/internalSession.ts`'s `readInternalOrigin()`, since Internal Suite page objects need absolute URLs.
