...
| ID | Test&nbsp;ID | Requirement | Status | Expected&nbsp;Behavior |
|---|---|---|---|---|
| REQ-AUTHZ-001 | TC-020 | A guest session cannot reach the standard REST API | ✅ Confirmed | _Last run: 7/31/2026, 3:19 PM — 1 browser._<br>A real guest session's cookies get a non-OK response hitting the standard REST API directly — the guest profile isn't granted API access. |
| REQ-AUTHZ-002 | TC-021 | A guest session cannot read a Case it does not own via the UI API (cross-record IDOR) | ✅ Confirmed | _Last run: 7/31/2026, 3:19 PM — 1 browser._<br>A guest session gets refused when it tries to read an arbitrary existing Case by Id — the guest profile has no broad read access or sharing rule on Case, only access to specific records it's explicitly granted. |
| REQ-AUTHZ-003 | TC-022 | A guest cannot set a field absent from the Create Case form via a tampered createRecord payload (mass assignment / Broken Object Property Level Authorization) | ✅ Confirmed | _Last run: 7/31/2026, 3:19 PM — 1 browser._<br>A guest tries to sneak an unauthorized field (`IsEscalated`) into a Case-creation request. Salesforce rejects the entire request outright instead of silently dropping the field — no Case gets created at all. |
<!-- LTM:AUTHZ:END -->
