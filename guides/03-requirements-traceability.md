...
<!-- LTM:SUMMARY:START -->
**Status:** 7/8 Guest Suite + 4/4 Internal Suite + 5/5 API Suite + 3/3 Penetration Suite requirements confirmed — matrix auto-generated 7/31/2026, 9:04 PM from the latest test run
<!-- LTM:SUMMARY:END -->
...
|---|---|---|---|---|
| REQ-CATALOG-001 | TC-001 | Product catalog loads; page 1 shows 9 of 16 products, paging to page 2 shows the remaining 7 | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>The page-count text updates before the tile list actually refetches, so the test polls for the tile count instead of asserting it once. |
| REQ-CATALOG-002 | TC-002 | Clicking a product shows its correct Name and MSRP | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>Clicking a tile doesn't navigate — it updates a detail panel in place on the same page. The test scopes its locators to that panel. |
| REQ-CATALOG-003 | TC-003, TC-004 | Category filter narrows the catalog to the exact expected count | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>The checkbox's native input is visually covered by a styled element, so clicking it directly never registers. Clicking the visible label text instead works. |
| REQ-CATALOG-004 | TC-005, TC-006 | Level filter narrows the catalog to the exact expected count | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>Same checkbox/label-click behavior as REQ-CATALOG-003. |
| REQ-CATALOG-005 | TC-007 | Search box narrows results by product name | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>— |
| REQ-CASE-002 | TC-008 | Submitting Create Case as a guest with all fields filled succeeds | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>Case creation succeeds, but Salesforce reassigns the new Case's owner to the org's default owner right after creation — required guest-sharing behavior, not a bug. That reassignment races the UI's success toast, which sometimes shows a cosmetic "resource does not exist" error even though the Case was created correctly. Confirmed via SOQL and by inspecting the failing request/response directly. The test checks the outgoing request payload instead of relying on the toast. |
| REQ-CASE-003 | TC-009 | Submitting Create Case with required fields empty shows validation and does not create a case | ❌ Known Gap | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>Subject and Description aren't actually required on this form — a blank submission creates a Case anyway. Confirmed via SOQL: blank-Subject Cases exist from real guest submissions. The test is wrapped in `test.fail()`, so a pass means the gap is still there; if it ever starts failing instead, that's the signal the form's validation was fixed. |
| REQ-GUEST-001 | TC-010 | Guest sees a Login option and no authenticated-only content | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 3 browsers._<br>The login control is a button labeled "Log in" (lowercase "in"), not a link labeled "Log In" as first assumed. |
<!-- LTM:GUEST:END -->
...
| ID | Test&nbsp;ID | Requirement | Status | Expected&nbsp;Behavior |
|---|---|---|---|---|
| REQ-AUTHZ-001 | TC-020 | A guest session cannot reach the standard REST API | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 1 browser._<br>A real guest session's cookies get a non-OK response hitting the standard REST API directly — the guest profile isn't granted API access. |
| REQ-AUTHZ-002 | TC-021 | A guest session cannot read a Case it does not own via the UI API (cross-record IDOR) | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 1 browser._<br>A guest session gets refused when it tries to read an arbitrary existing Case by Id — the guest profile has no broad read access or sharing rule on Case, only access to specific records it's explicitly granted. |
| REQ-AUTHZ-003 | TC-022 | A guest cannot set a field absent from the Create Case form via a tampered createRecord payload (mass assignment / Broken Object Property Level Authorization) | ✅ Confirmed | _Last run: 7/31/2026, 9:01 PM — 1 browser._<br>A guest tries to sneak an unauthorized field (`IsEscalated`) into a Case-creation request. Salesforce rejects the entire request outright instead of silently dropping the field — no Case gets created at all. |
<!-- LTM:AUTHZ:END -->
